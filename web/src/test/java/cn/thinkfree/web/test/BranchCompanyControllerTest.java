...
@WebAppConfiguration
public class BranchCompanyControllerTest extends AbsControllerTest {
    ...
    public void list() throws Exception {
        String rs = mvc.perform(get("/branchCompany/branchCompanylist")
                .param("legalName","18210142179")
                .with(user(userVO))
        )
                .andExpect(status().isOk())
                .andDo(print())         //打印出请求和相应的内容
                .andReturn().getResponse().getContentAsString();
        MyRespBundle<PageInfo<BranchCompanyVO>> rsb = gson.fromJson(rs,  new TypeToken<MyRespBundle<PageInfo<BranchCompanyVO>>>() {}.getType());
